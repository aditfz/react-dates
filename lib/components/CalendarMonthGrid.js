...
function getMonths(initialMonth, numberOfMonths, withoutTransitionMonths) {
  ...
  for (var i = 0; i < (withoutTransitionMonths ? numberOfMonths : numberOfMonths + 2); i += 1) {
    month = momentJalaali(month).startOf('jMonth')
    months.push(month);
    // month = month.clone().startOf('month')
    newMonth = momentJalaali(month.clone()).add(1, 'jMonth');
    if(momentJalaali(newMonth).startOf('jMonth').format('jMM') === momentJalaali(month).startOf('jMonth').format('jMM') ){
      newMonth = momentJalaali(newMonth.clone()).add(1, 'jMonth');
    }
    month = newMonth
  }
  ...
  _proto.componentWillReceiveProps = function componentWillReceiveProps(nextProps) {
    ...
    if (hasMonthChanged && !hasNumberOfMonthsChanged) {
      if ((0, _isNextMonth["default"])(prevInitialMonth, initialMonth)) {
        ...
        var newAddedMonth = months[months.length - 1].clone().add(1, 'month')
        console.log('1', momentJalaali(newAddedMonth).startOf('jMonth'))
        console.log('2', momentJalaali(months[months.length - 1].clone()).startOf('jMonth'))
        if(momentJalaali(newAddedMonth).startOf('jMonth').format('jMM') === momentJalaali(months[months.length - 1].clone()).startOf('jMonth').format('jMM') ){
          newAddedMonth = newAddedMonth.clone().add(1, 'month')
        }
        newMonths.push(newAddedMonth);
      } else if ((0, _isPrevMonth["default"])(prevInitialMonth, initialMonth)) {
        newMonths = months.slice(0, months.length - 1);
        var newAddedMonth = months[0].clone().subtract(1, 'month')
        if(momentJalaali(newAddedMonth).startOf('jMonth').format('jMM') === momentJalaali(months[0].clone()).startOf('jMonth').format('jMM') ){
          newAddedMonth = newAddedMonth.clone().subtract(1, 'month')
        }
        newMonths.unshift(newAddedMonth);
      } else {
